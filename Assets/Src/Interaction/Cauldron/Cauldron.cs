...
namespace Src.Interaction.Cauldron
{
	public class Cauldron : MonoBehaviour, IInteractable
	{
		...
		private readonly InventoryStates[] _recipeOne =
			{ InventoryStates.Red, InventoryStates.Purple, InventoryStates.Green, InventoryStates.Green };

		private readonly InventoryStates[] _recipeTwo =
		{
		...
		private void AddIngredient(InventoryStates ingredient)
		{
			...
			if (_currentIngredients.Count > Mathf.Max(_recipeOne.Length, _recipeTwo.Length))
			{
				Debug.LogError("Добавлено слишком много ингредиентов! Игрок умирает.");
				return;
			}

			if (CheckRecipe(_recipeOne))
			{
				if (_currentIngredients.Count == _recipeOne.Length)
				{
					Debug.Log("Рецепт 1 собран! Варим яд.");
					_animator.SetTrigger(s_poison);
					_cauldronState = CauldronStates.Potion;
					return;
				}
			}
			else if (CheckRecipe(_recipeTwo))
			{
				if (_currentIngredients.Count == _recipeTwo.Length)
				{
					Debug.Log("Рецепт 2 собран! Варим супер-яд.");
			...
			{
				Debug.LogError("Неправильные ингредиенты! Игрок умирает.");
				return;
			}
		...
		private void TakePotion()
		{
			...
			_characterInventory.ChangeSlot(
				_currentIngredients.Count == _recipeOne.Length ? InventoryStates.Poison : InventoryStates.ExtraPoison);
